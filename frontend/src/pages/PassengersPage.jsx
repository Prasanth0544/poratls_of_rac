...
function PassengersPage({ trainData, onClose, onNavigate }) {
      ...
      coach.berths.forEach(berth => {
        ...
        if (berth.segmentOccupancy && berth.segmentOccupancy[currentIdx] === null) {
          ...
          vacant.push({
            ...
            vacantToStation: vacantToIdx < stations.length ? stations[vacantToIdx]?.code : 'END',
            vacantFromStationName: stations[vacantFromIdx]?.name || 'N/A',
            vacantToStationName: vacantToIdx < stations.length ? stations[vacantToIdx]?.name : 'Journey End'
          });
        }
      });
    });
  ...
  const filterVacantBerths = () => {
    ...
    if (vacantFromStation.trim()) {
      filtered = filtered.filter(berth => 
        berth.vacantFromStation.toLowerCase().includes(vacantFromStation.toLowerCase()) ||
        berth.vacantFromStationName.toLowerCase().includes(vacantFromStation.toLowerCase())
      );
    }
    ...
    if (vacantToStation.trim()) {
      filtered = filtered.filter(berth => 
        berth.vacantToStation.toLowerCase().includes(vacantToStation.toLowerCase()) ||
        berth.vacantToStationName.toLowerCase().includes(vacantToStation.toLowerCase())
      );
    }
  ...
  const applyFilters = () => {
    ...
    switch(filterStatus) {
      case 'cnf':
        filtered = filtered.filter(p => p.pnrStatus === 'CNF' && !p.pnrStatus.includes('RAC'));
        break;
      case 'rac-cnf':
        filtered = filtered.filter(p => 
        ...
        break;
      case 'rac':
        filtered = filtered.filter(p => p.pnrStatus && p.pnrStatus.startsWith('RAC'));
        break;
      case 'boarded':
      ...
      {trainData && trainData.journeyStarted && (
        <div className="vacant-toggle-section">
          ...
          >
            ...
            {!showVacantBerths && (
              <span className="toggle-count">({vacantBerths.length} vacant at {trainData.stations[trainData.currentStationIdx]?.code})</span>
            )}
          </button>
        </div>
      )}
      ...
      {showVacantBerths && trainData && trainData.journeyStarted && (
        <div className="vacant-berths-section">
          <div className="section-header">
            <h3>🛏️ Vacant Berths at {trainData.stations[trainData.currentStationIdx]?.name}</h3>
              ...
              {filteredVacantBerths.length === 0 ? (
                ...
                <div className="table-container">
                  <div className="filter-result-info">
                    Showing <strong>{filteredVacantBerths.length}</strong> of <strong>{vacantBerths.length}</strong> vacant berths
                  </div>
                  <table className="vacant-berths-table">
                    ...
                    <tbody>
                      ...
                      <td className="td-station">
                        <span className="station-code current">{berth.currentStation}</span>
                      </td>
                      <td className="td-station">
                        <span className="station-code vacant-from" title={berth.vacantFromStationName}>
                          {berth.vacantFromStation}
                        </span>
                      </td>
                      <td className="td-station">
                        <span className="station-code vacant-to" title={berth.vacantToStationName}>
