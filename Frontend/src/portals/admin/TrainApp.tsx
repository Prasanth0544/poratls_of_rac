...
function App({ initialPage, onLogout }: AppProps): React.ReactElement {
      ...
      try {
        ...
        if (response && response.success && response.data) {
          ...
          if (!backendInitialized) {
            ...
            // Don't set trainData so auto-init effect can trigger
            console.log("[App] Train not initialized in backend memory, will auto-initialize...");
            setCurrentPage(restoredPage);
            setJourneyStarted(false);
            setAutoInitAttempted(false); // Allow auto-init to fire
          } else {
  ...
  const handleReset = async (): Promise<void> => {
    ...
    try {
      ...
      if (response.success) {
        ...
        stopTimerPolling();
        setTimerSeconds(TIMER_DURATION);
        await clearAppState(urlTrainNo); // Clear persisted state on reset
        await loadTrainState();
  ...
  const handleLogout = async (): Promise<void> => {
    ...
    await clearAppState(urlTrainNo); // Clear persisted state on logout
      ...
      <div className="App app-classic">
        ...
        <div className="app-content">
          <ConfigPage
            ...
            onApplySuccess={initialPage === 'config' ? (trainNo: string) => { window.location.href = `/admin/train/${trainNo}`; } : undefined}
